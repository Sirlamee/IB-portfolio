...
function splitScroll() {
    ...
    if (window.innerWidth <= 950) {
        ...
        sections.forEach(section => {
            ...
            columnB.style.flex = '';
            columnB.style.opacity = '';  // Clear any opacity interference
    ...
    sections.forEach(section => {
        ...
        // Scale column B content but preserve hover functionality
        const scale = 1 + (progress * 0.2);
        columnBContent.style.transform = `scale(${scale})`;
        
        // Ensure column B maintains its opacity for hover effects
        columnB.style.opacity = '';  // Clear any opacity interference
    });
}
